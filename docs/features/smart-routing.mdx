...
<AccordionGroup>
  ...
  <Accordion title="Embedding Model Selection">
    Different embedding models produce different vector dimensions. Choose based on your performance and accuracy needs:

    | Model | Dimensions | Index Type | Notes |
    |-------|------------|------------|-------|
    | `text-embedding-3-small` | 1536 | HNSW | **Recommended** - Best balance of quality and performance |
    | `text-embedding-3-large` | 3072 | HNSW (halfvec) | Higher quality, requires pgvector ≥ 0.7.0 |
    | `bge-m3` | 1024 | HNSW | Open-source alternative |
    | `gemini-embedding-001` | 3072 | HNSW (halfvec) | Google Gemini — free tier (100 RPM), OpenAI-compatible endpoint, requires pgvector ≥ 0.7.0 |

    <Info>
      **HNSW is the Recommended Index Type**
      
      MCPHub uses HNSW indexes by default because:
      - Better query performance than IVFFlat
      - Can be created immediately without pre-existing data
      - More robust as data distribution changes
    </Info>

    <Warning>
      **Vector Dimension Limits (pgvector 0.7.0+)**
      
      - `vector` type: up to **2,000 dimensions** (HNSW or IVFFlat)
      - `halfvec` type: up to **4,000 dimensions** (used automatically for high-dimensional embeddings)
      
      For models with 3072 dimensions (`text-embedding-3-large`, `gemini-embedding-001`):
      - Requires pgvector ≥ 0.7.0 for halfvec support
      - MCPHub automatically uses halfvec casting for optimal indexing
      - Older pgvector versions will work but without index optimization
      
      **Recommendations:**
      1. Upgrade to pgvector 0.7.0+ for best high-dimensional embedding support
      2. Or use `text-embedding-3-small` (1536 dimensions) for broader compatibility
    </Warning>
    ...
    # Higher quality (requires pgvector >= 0.7.0 for optimal indexing)
    EMBEDDING_MODEL=text-embedding-3-large

    # Google Gemini via OpenAI-compatible endpoint (free tier: 100 RPM) (requires pgvector >= 0.7.0 for optimal indexing)
    ...
    <Tip>
      **Google Gemini Free Tier**
      ...
      `gemini-embedding-001` is available via Google's OpenAI-compatible endpoint at
      `https://generativelanguage.googleapis.com/v1beta/openai`.
      It operates through the standard `openaiApiBaseUrl` setting — no additional
      configuration is required beyond setting the base URL, API key, and model name.
      Google provides a generous **free tier of 100 RPM**, making it an excellent
      choice for self-hosted or low-traffic deployments.
    </Tip>
